...
#'
#' Helper function to add amino acid to codon.
#'
#' @return Corresponing amino acid\(s\)
#' @keywords internal
.get_codon_aa_map <- function() {
    c(
...
#' @return Tibble with columns codon, frequency, aa
#' @export
#'
#' @examples
#' freqs <- calculate_codon_frequencies("ATGATGATG")
#' codon_frequency_df(freqs)
codon_frequency_df <- function(frequencies) {
    stopifnot(
        is.numeric(frequencies),
        !is.null(names(frequencies))
    )

    codons <- names(frequencies)

    data.frame(
        codon = factor(codons, levels = codons),
